...
metro_bounds <- function(state, metro) {
    ...
    core_based_statistical_areas(cb = TRUE, year = 2010, resolution = `500k`) %>% 
...
county_tract_bounds <- function(geography, state, county) {
  ...
  if (geography == "county") {
    return(
      left_join(
        bounds, 
        county_outcomes_ez %>% select(state, county, contains("pooled")),
        by = c("state", "county")
      )
    )
  }
...
```{r}
tmap_region(geography = "county", state = "California", outcome = "kfr_pooled_pooled_p25")
```
